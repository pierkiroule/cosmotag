...
const leafProfiles = [
  {
    ...
    vein: 'rgba(224, 242, 254, 0.75)',
    spark: 'rgba(236, 254, 255, 0.45)',
  },
  {
    shape: 'auric',
    border: 'rgba(251, 191, 36, 0.55)',
    vein: 'rgba(254, 243, 199, 0.8)',
    spark: 'rgba(253, 230, 138, 0.45)',
  },
  {
    shape: 'fable',
    border: 'rgba(129, 140, 248, 0.55)',
    vein: 'rgba(199, 210, 254, 0.75)',
    spark: 'rgba(196, 181, 253, 0.4)',
  },
];

const getLeafProfile = () => leafProfiles[Math.floor(Math.random() * leafProfiles.length)];

const createLeafVisual = () => {
  const palette = getRandomPalette();
  const profile = getLeafProfile();
  ...
  const glintDelay = Math.random() * 8;
  const veinOffset = `${(Math.random() * 6 - 3).toFixed(2)}%`;
